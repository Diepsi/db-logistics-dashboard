    ...
    <header class="sticky top-0 z-50 bg-white/80 backdrop-blur-md border-b border-slate-200 shadow-sm">
        <nav class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div class="flex items-center justify-between h-16 min-h-[44px]">
                ...
                <!-- Logo -->
                <a href="{{ route('home') }}" class="flex items-center space-x-3 py-2">
                    <div class="w-10 h-10 rounded-xl bg-gradient-to-br from-anl-blue to-anl-navy flex items-center justify-center text-white font-extrabold text-lg shadow-md">
                        ANL
                    </div>
                    <div class="leading-tight">
                        <span class="text-anl-navy font-extrabold tracking-wide block">LOGISTICS</span>
                ...
                <div class="hidden lg:flex items-center space-x-1">
                    ...
                    @foreach ($menuItems as [$key, $label, $url, $activeOn])
                        <a href="{{ $url }}" class="px-3 xl:px-4 py-2 min-h-[44px] flex items-center rounded-lg text-sm font-semibold transition-all duration-300 {{ request()->routeIs($activeOn) ? 'text-anl-blue' : 'text-slate-600 hover:text-anl-blue' }}">
                            {{ $label }}
                        </a>
                    @endforeach
                </div>
                ...
                <div class="hidden lg:flex items-center space-x-4">
                    <a href="https://wa.me/6281362323510" target="_blank" rel="noopener"
                       class="flex items-center gap-2 text-sm font-semibold text-slate-700 hover:text-anl-blue transition-colors">
                        ...
                        <span class="text-xs text-slate-500">0813-6232-3510</span>
                    </a>
                    <a href="{{ route('contact') }}"
                       class="px-5 py-2.5 min-h-[44px] flex items-center rounded-lg bg-anl-blue text-white font-semibold shadow-md shadow-anl-blue/20 hover:bg-anl-blue-dark transition-all duration-300">
                ...
                <div class="lg:hidden" x-data="{ open: false }">
                    <button @click="open = !open" class="min-h-[44px] min-w-[44px] flex items-center justify-center text-anl-navy hover:text-anl-blue focus:outline-none transition-colors">
                    ...
                    <div x-show="open" @click.away="open = false"
                         ...
                         class="absolute right-4 top-16 w-64 bg-white rounded-2xl shadow-xl border border-slate-200 py-2 z-50">
                        @foreach ($menuItems as [$key, $label, $url, $activeOn])
                            <a href="{{ $url }}" class="block px-4 py-3 min-h-[44px] text-sm font-semibold {{ request()->routeIs($activeOn) ? 'text-anl-blue' : 'text-slate-700 hover:bg-slate-50 hover:text-anl-blue' }}">{{ $label }}</a>
                        @endforeach
                        <div class="border-t border-slate-100 my-2"></div>
                        <a href="https://wa.me/6281362323510" target="_blank" rel="noopener"
                           class="block px-4 py-3 min-h-[44px] text-sm font-semibold text-green-600 hover:bg-slate-50">WhatsApp: 0813-6232-3510</a>
                    </div>
                </div>
            </div>
        </nav>
