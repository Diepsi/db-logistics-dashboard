...
<body class="site-root antialiased bg-slate-50 text-slate-600" x-data="{ backTop: false }" @scroll.window="backTop = window.scrollY > 300">
    ...
    <header class="sticky top-0 z-50" x-data="{ scrolled: false, open: false }" @scroll.window="scrolled = window.scrollY > 8">
        <nav class="bg-anl-navy/90 backdrop-blur-md border-b border-white/10 transition-shadow duration-300" :class="scrolled ? 'shadow-lg shadow-black/30' : 'shadow-sm'">
            <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                <div class="flex items-center justify-between h-16">
                    ...
                    <div class="hidden lg:flex items-center justify-center flex-1">
                        <div class="flex items-center gap-1">
                            ...
                            @foreach ($menuItems as [$key, $label, $url, $activeOn])
                                ...
                                @endphp
                                <a href="{{ $url }}"
                                   class="px-4 py-2 min-h-[44px] flex items-center rounded-full text-sm font-semibold transition-all duration-200 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-anl-amber/50 {{ $active ? 'bg-white/10 text-anl-amber' : 'text-slate-300 hover:bg-white/10 hover:text-white' }}"
                    ...
                    <div class="lg:hidden flex items-center gap-2">
                        ...
                        <button @click="open = !open" :aria-expanded="open.toString()" aria-controls="mobile-menu" aria-label="Menu navigasi"
                                class="w-11 h-11 rounded-lg flex items-center justify-center text-white hover:bg-white/10 hover:text-anl-amber transition-colors duration-200">
            ...
            <div id="mobile-menu" x-show="open" x-cloak @click.away="open = false" @keydown.escape.window="open = false"
                 ...
                 class="lg:hidden bg-anl-navy-dark border-t border-white/10 px-4 sm:px-6 pb-4 pt-3">
                <div class="flex items-center gap-2 pb-3 mb-1 border-b border-white/10">
                    ...
                    <div class="leading-tight">
                        <span class="text-white font-extrabold text-xs tracking-tight block">AMANAH NUSANTARA</span>
                        <span class="text-[10px] font-bold tracking-[0.2em] text-slate-400 block">LOGISTIK<span class="text-anl-amber">.</span></span>
                    </div>
                </div>
                <nav aria-label="Navigasi mobile" class="pt-2">
                    @foreach ($menuItems as [$key, $label, $url, $activeOn])
                        ...
                        @endphp
                        <a href="{{ $url }}"
                           class="block px-4 py-2.5 min-h-[44px] flex items-center rounded-xl text-sm font-semibold transition-colors duration-200 {{ $active ? 'bg-white/10 text-anl-amber' : 'text-slate-300 hover:bg-white/5 hover:text-white' }}"
                           @if ($active) aria-current="page" @endif>
                            {{ $label }}
                        </a>
                    @endforeach
                ...
                <div class="mt-3 pt-3 border-t border-slate-100 flex flex-col gap-2">
                    ...
                    <a href="https://wa.me/6281362323510" target="_blank" rel="noopener" class="block text-center py-3 min-h-[44px] rounded-xl border border-green-500/40 text-green-400 text-sm font-bold hover:bg-white/5 transition-colors duration-200">
    ...
    <button type="button" x-show="backTop" @click="window.scrollTo({ top: 0, behavior: 'smooth' })"
            x-cloak x-transition:enter="transition ease-out duration-200"
            x-transition:enter-start="opacity-0 translate-y-3" x-transition:enter-end="opacity-100 translate-y-0"
            x-transition:leave="transition ease-in duration-150" x-transition:leave-start="opacity-100"
            x-transition:leave-end="opacity-0"
            aria-label="Kembali ke atas"
            class="fixed bottom-6 left-6 z-50 w-12 h-12 min-w-[44px] min-h-[44px] flex items-center justify-center rounded-full bg-anl-navy text-anl-amber shadow-lg shadow-black/30 border border-white/10 hover:bg-anl-navy-light hover:-translate-y-0.5 transition-all duration-300">
